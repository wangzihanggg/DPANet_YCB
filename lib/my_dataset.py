...
class YCBData(Dataset):
    def __init__(self, cfg):
        ...
        if cfg.mode == "train":
            ...
            def read_test(name):
                test_path = "/project/1_2301/DPANet-master/YCB_Video_Dataset/data"
                """
                data
                    -- 0000
                        -- many rgb depth and mask
                """
                samples = []
                sub_dirs = os.listdir(test_path)
                for sub_dir in sub_dirs:
                    dir_path = os.path.join(test_path, sub_dir)
                    all_files = os.listdir(dir_path)
                    for file in all_files:
                        if "color" in file:
                            rgb_path = os.path.join(dir_path, file)
                            depth_path = os.path.join(dir_path, file[:6] + "-depth.png")
                            mask_path = os.path.join(dir_path, file[:6] + "-label.png")
                            samples.append([rgb_path, depth_path, mask_path])
                # samples = []
