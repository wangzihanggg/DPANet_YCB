...
class DataPrefetcher(object):
    ...
    def preload(self):
        try:
            if len(self.list) == 0 or len(self.list) >= self.cnt:
                ...
                self.next_input, self.next_depth, self.next_target, self.next_img_name = next(self.loader)
        ...
        with torch.cuda.stream(self.stream):
            ...
            # self.next_img_name[idx] = self.next_img_name[idx].cuda(non_blocking=True)
            self.idx = idx
            self.pre_idx += 1
    ...
    def next(self):
        ...
        img_name = self.next_img_name[self.idx]
        # gate = self.next_gate[self.idx]
        self.preload()
        # return input, depth, target, gate
        # return input, depth, target
        return input, depth, target, img_name
