...
class YCBData(Dataset):
    def __init__(self, cfg):
        ...
        if cfg.mode == "train":
            ...
            for sub_dir in sub_dirs:
                ...
                for file in all_files:
                    if "color" in file:
                        ...
                        self.samples.append([rgb_path, depth_path, mask_path])

            train_path_2 = "/project/1_2301/DPANet-master/YCB_Video_Dataset/data_syn"
            sub_dirs = os.listdir(train_path_2)
            for sub_dir in sub_dirs:
                dir_path = os.path.join(train_path_2, sub_dir)
                all_files = os.listdir(dir_path)
                for file in all_files:
                    if "color" in file:
                        rgb_path = os.path.join(dir_path, file)
            ...
            # self.samples = self.samples[-100:]  # data syn
    ...
    def __getitem__(self, idx):
        ...
        image_name, depth_name, mask_name = self.samples[idx]
        ...
        if self.mode == "train":
            ...
            return image, depth, mask, image_name
        else:
...
def train_collate_fn(batch):
    ...
    # images, depths, masks = zip(*batch)
    images, depths, masks, img_name = zip(*batch)
    l = len(images[0])
    ...
    return images_t, depths_t, masks_t, img_name_t
...
if __name__ == '__main__':
    ...
    for i in range(3):
        ...
        rgb, depth, mask= prefetcher.next()
